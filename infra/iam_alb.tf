...
#
# The controller runs in-cluster and calls the EC2/ELB APIs to create the ALB
# that fronts Argo CD and ShopFast. It authenticates through the cluster's OIDC
# provider (IRSA) rather than node-wide credentials, so only this ServiceAccount
# can manage load balancers.
# ---------------------------------------------------------------------------
...
# Policy mirrors the upstream AWSLoadBalancerControllerIAMPolicy published by the
# aws-load-balancer-controller project. Kept inline (rather than fetched over the
# network at apply time) so provisioning is deterministic and offline-safe.
